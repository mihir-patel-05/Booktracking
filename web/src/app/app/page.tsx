...
export default async function HomePage() {
  ...
  const [{ data: books }, { data: stats }] = await Promise.all([
    ...
    supabase.from("user_stats").select("current_streak,total_xp,level,minutes_read").maybeSingle(),
  ]);
  ...
  return (
    <>
      ...
      <section className="grid gap-3 sm:grid-cols-3" aria-label="Reading summary">
        ...
        <Metric icon={<BookOpen size={19} />} label="Minutes read" value={String(stats?.minutes_read ?? 0)} />
        <Metric label={`Level ${stats?.level ?? 1}`} value={`${stats?.total_xp ?? 0} XP`} />
      </section>
